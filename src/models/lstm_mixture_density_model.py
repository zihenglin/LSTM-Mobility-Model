...
class tf_lstm_mixture_density_model:
    ...
    def init_model(self):
        with tf.variable_scope(self.name):
            ...
            self.X = tf.placeholder(
                "float", [None, self.pred_x_dim])

            # Input value sequence
            self.input_X = tf.placeholder(
                "float", [None, self.input_length, self.obs_x_dim])

            # True sequence for calculating loss
            self.y = tf.placeholder(
                "float", [None, self.input_length, self.y_dim])

            # Bias for sampling
            self.location_sd_bias = tf.placeholder("float", [1], name='sd_bias')
            self.time_sd_bias = tf.placeholder("float", [1], name='sd_bias')
            self.pi_bias = tf.placeholder("float", [1], name='pi_bias')

            # Affine weights
            self.W1 = W1 = tf.Variable(tf.random_normal([self.n_lstm_units, self.output_dim],
                                                        stddev=self.init_stdev,
                                                        dtype=tf.float32))
            self.b1 = b1 = tf.Variable(tf.random_normal([1, self.output_dim],
                                                        stddev=self.init_stdev,
                                                        dtype=tf.float32))

            # Multi-layer lstm cells with dropouts
            lstm_cell = rnn_cell.BasicLSTMCell(self.n_lstm_units,
                                               forget_bias=0.0,
                                               state_is_tuple=True)

            lstm_cell = rnn_cell.DropoutWrapper(
                lstm_cell, output_keep_prob=1 - self.dropout_prob)
        ...
        states = tf.reshape(states, [self.input_length,
                                     self.batch_size,
                                     self.n_lstm_units * self.n_layers * 2])
        states = tf.transpose(states, [1, 0, 2])

        self.loss = self.sequence_loss(self.y, outputs)
    ...
    def sample_obs(self,
        ...
        prob_st = self.tf_normal(tf.tile(self.time[time_step],
                                         [1, self.n_dur_mixtures]),
                                 mu_st,
                                 s_st)
        ...
        ind = tf.concat(
            1, [np.array(range(self.batch_size)).reshape(self.batch_size, 1), ind])

        # Cast to int32 in order to work with indexing requirement
        ind = tf.cast(ind, tf.int32)

        # Sample location
        sampled_x, sampled_y = self.sample_mixture_2d(ind,
                                                      mu_locx,
                                                      mu_locy,
                                                      s_locx,
                                                      s_locy,
        ...
        sample_dur = self.sample_mixture_1d(ind,
                                            mu,
                                            s,
                                            self.n_dur_mixtures)
        cur_time = self.time[time_step]
        # time_step = tf.Print(time_step, [time_step], "time_step")
        # cur_time = tf.Print(cur_time, [cur_time, time_step], "cur_time")
        # sample_dur = tf.Print(sample_dur, [sample_dur], "sample_dur")
        output = tf.concat(1, [sampled_x, sampled_y, cur_time, sample_dur])

        # output = tf.Print(output, [output[:, 2]], 'output')

        self.time[time_step + 1] = cur_time + sample_dur

        b = tf.reshape(tf.gather_nd(finished, ind), [self.batch_size, 1])
        a = tf.gather_nd(out_category, ind)
        coef = tf.concat(1, [a, b])

        mixture_coef = tf.concat(1, [tf.reshape(tf.gather_nd(pi, ind),
                                                [self.batch_size, 1]),
                                     tf.reshape(tf.gather_nd(mu_locx, ind),
                                                [self.batch_size, 1]),
                                     tf.reshape(tf.gather_nd(mu_locy, ind),
                                                [self.batch_size, 1]),
                                     tf.reshape(tf.gather_nd(s_locx, ind),
                                                [self.batch_size, 1]),
                                     tf.reshape(tf.gather_nd(s_locy, ind),
                                                [self.batch_size, 1]),
                                     tf.reshape(tf.gather_nd(mu_st, ind),
                                                [self.batch_size, 1]),
                                     tf.reshape(tf.gather_nd(mu_dur, ind),
                                                [self.batch_size, 1]),
                                     tf.reshape(tf.gather_nd(s_st, ind),
                                                [self.batch_size, 1]),
                                     tf.reshape(tf.gather_nd(s_dur, ind),
                                                [self.batch_size, 1]),
                                     tf.reshape(tf.gather_nd(rho, ind),
                                                [self.batch_size, 1])]
                                 )
    ...
    def get_mixture_coef(self, output):
        ...
        if len(output.get_shape()) == 2:
            ...
            dur = output[:, self.n_loc_mixtures * self.loc_mixture_ratio:
                         self.n_loc_mixtures * self.loc_mixture_ratio +
                         self.n_dur_mixtures * self.dur_mixture_ratio]
            ...
            out_category = output[:, self.n_loc_mixtures * self.loc_mixture_ratio +
                                  ...
                                  self.n_loc_mixtures * self.loc_mixture_ratio +
                                  self.n_dur_mixtures * self.dur_mixture_ratio +
                                  self.n_categories * self.n_loc_mixtures]

            out_finished = output[:, self.n_loc_mixtures * self.loc_mixture_ratio +
                                  self.n_dur_mixtures * self.dur_mixture_ratio +
                                  self.n_categories * self.n_loc_mixtures:]

            pi_loc, mu_locx, mu_locy, s_locx, s_locy = tf.split(
                1, self.loc_mixture_ratio, loc)

            mu_st, mu_dur, s_st, s_dur, rho = tf.split(
                1, self.dur_mixture_ratio, dur)
            ...
            pi_loc, mu_locx, mu_locy, s_locx, s_locy = tf.split(
                2, self.loc_mixture_ratio, loc)

            mu_st, mu_dur, s_st, s_dur, rho = tf.split(
                2, self.dur_mixture_ratio, dur)
    ...
    def sequence_loss(self, y, pred_seq):
        ...
        loss = self.mdn_lossfunc(self.y[:, :, 0],
                                 self.y[:, :, 1],
                                 self.y[:, :, 2],
                                 self.y[:, :, 3],
                                 self.y[:, :, 4:7],
    ...
    def mdn_lossfunc(self,
        ...
        loc_x = tf.reshape(
            loc_x, [self.batch_size, self.input_length, 1])
        loc_y = tf.reshape(
            loc_y, [self.batch_size, self.input_length, 1])
        st = tf.reshape(
            st, [self.batch_size, self.input_length, 1])
        dur = tf.reshape(
            dur, [self.batch_size, self.input_length, 1])
        category = tf.reshape(
            category, [self.batch_size, self.input_length, 1, self.n_categories])
        finished = tf.reshape(
            finished, [self.batch_size, self.input_length, self.n_finished])
        ...
        # Loss due to loc
        prob_loc = self.tf_2d_normal(loc_x,
                                     loc_y,
                                     mu_locx,
                                     mu_locy,
                                     s_locx,
        ...
        # Loss due to dur
        prob_dur = self.tf_2d_normal_correlation(st,
                                                 dur,
                                                 mu_st,
                                                 mu_dur,
                                                 s_st,
        ...
        # Loss due to category
        prob_finished = (tf.mul(out_finished, finished) +
                         tf.mul(1 - out_finished, 1 - finished))
    ...
    def generate_sequence_coefficients(self,
        ...
        for i in xrange(n):
            ...
            seq, coef, mixture_coef, state = sess.run([self.gen_seq, self.coefficients, self.mixture_coefficients, self.states],
                                                      feed_dict={self.X: X_init[rand_ind],
                                                                 self.input_X: X_input_seq[rand_ind],
                                                                 self.time[0]: st_sample,
                                                                 self.location_sd_bias: [location_sd_bias],
            ...
            seq = np.concatenate((seq, X_input_seq[rand_ind, :, 0][:, :, np.newaxis]), 2)
            gen_seq.append(seq)
        ...
        gen_seq = gen_seq.reshape(
            n * self.batch_size, self.input_length, self.pred_x_dim + 1)
        gen_coef = np.array(gen_coef)
        gen_coef = gen_coef.reshape(
            n * self.batch_size, self.input_length, self.n_probabilities)
        gen_mixture_coef = np.array(gen_mixture_coef)
        gen_mixture_coef = gen_mixture_coef.reshape(
            n * self.batch_size, self.input_length, -1)
        gen_states = np.array(gen_states)
        return gen_seq, gen_coef, gen_states, gen_mixture_coef

    def get_category(self, category):
        if len(category.get_shape()) == 2:
            return tf.nn.softmax(tf.reshape(
    ...
    def get_finished(self, finished):
        return tf.sigmoid(
            tf.clip_by_value(finished, -20, 20))
    ...
    def tf_2d_normal_correlation(self,
                                 x1,
                                 x2,
                                 mu1,
                                 mu2,
